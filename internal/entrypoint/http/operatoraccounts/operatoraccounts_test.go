...
func newCanonicalOperatorHandler(provider principal.Provider) http.Handler {
	return NewWithPhoneAuth(
		canonicalStartProbe{},
		canonicalCodeProbe{},
...
func TestOperatorAccountHTTPConcurrentRequests(t *testing.T) {
	...
	handler := newCanonicalOperatorHandler(provider)
	jar := newCookieJar()
	...
	for index := range 32 {
		...
		go func(index int) {
			...
			local := newCookieJar()
			jar.mu.Lock()
			maps.Copy(local.cookies, jar.cookies)
			jar.mu.Unlock()
			if index%2 == 0 {
...
func TestOperatorAccountRouteIsRegisteredBehindPrincipalAndDisabledWithoutLiveAuth(t *testing.T) {
	...
	unauthedHandler := NewWithPhoneAuth(nil, nil, nil, nil, nil, unauthenticated, "https://example.test", RouteOptions{Mode: RouteDisabled, Cookie: SecureCookieConfig()})
...
func TestOperatorAccountDevelopmentTestMockRequiresExplicitOptIn(t *testing.T) {
	...
	NewWithPhoneAuth(canonicalStartProbe{}, canonicalCodeProbe{}, canonicalPasswordProbe{}, canonicalCancelProbe{}, canonicalStatusProbe{}, provider, "https://example.test", RouteOptions{Mode: RouteDevelopmentTestMock, Cookie: SecureCookieConfig()})
}
...
func TestOperatorAccountCookiesFollowDeploymentPolicy(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			handler := NewWithPhoneAuth(canonicalStartProbe{}, canonicalCodeProbe{}, canonicalPasswordProbe{}, canonicalCancelProbe{}, canonicalStatusProbe{}, provider, test.origin, RouteOptions{Mode: RouteDevelopmentTestMock, Environment: EnvironmentTesting, Cookie: test.cookie, AllowDevelopmentTestMock: true})
...
func TestOperatorAccountCookiePolicyRejectsImplicitInsecureAndWrongHostSettings(t *testing.T) {
	...
	NewWithPhoneAuth(canonicalStartProbe{}, canonicalCodeProbe{}, canonicalPasswordProbe{}, canonicalCancelProbe{}, canonicalStatusProbe{}, provider, "http://dev.example.test", RouteOptions{Mode: RouteDevelopmentTestMock, Environment: EnvironmentDevelopment, Cookie: LocalInsecureCookieConfig(), AllowDevelopmentTestMock: true})
}

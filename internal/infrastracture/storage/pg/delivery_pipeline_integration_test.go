...
func createDeliveryPipelineFixture(
	...
	fixture.deliveries = make([]pipelineDelivery, 0, 4)
...
func readDeliveryState(
	...
	if failure := database.QueryRow(
		context,
		`SELECT status::text, attempt_count, COALESCE(error_message, ''), COALESCE(lease_token, '00000000-0000-0000-0000-000000000000')
		 FROM mailing_deliveries
		 WHERE mailing_id = $1 AND run_id = $2 AND recipient_id = $3`,
		item.mailingID,
		item.runID,
		item.recipientID,
	).Scan(&state.status, &state.attemptCount, &state.errorMessage, &state.leaseToken); failure != nil {

...
func (command StartPhoneCommand) Execute(
	...
	challenge := application.PhoneChallenge{
		RequestID: uuid.NewSHA1(mockNamespace, []byte(command.store.nextIDLocked("phone"))),
		Phone:     normalized,
		Delivery:  "Telegram SMS",
		ExpiresAt: time.Now().Add(phoneChallengeLifetime),
	}
	command.store.phone = &phoneChallengeState{
		challenge: challenge,
		code:      MockPhoneCode,
	}
...
func (command VerifyPhoneCommand) Execute(
	...
	if command.store.phone == nil || command.store.phone.challenge.RequestID != requestID {
		return application.Account{}, fmt.Errorf("%w: phone request %s", ErrChallengeNotFound, requestID)
	}
	if !time.Now().Before(command.store.phone.challenge.ExpiresAt) {
		command.store.phone = nil
		return application.Account{}, fmt.Errorf("%w: phone request %s", ErrChallengeExpired, requestID)
	}
	if strings.TrimSpace(code) != command.store.phone.code {
		return application.Account{}, fmt.Errorf("%w for phone request %s", ErrInvalidCode, requestID)
	}

	phone := command.store.phone.challenge.Phone
	command.store.phone = nil
	for _, account := range command.store.accounts {
	...
	account := application.Account{
		ID:                uuid.NewSHA1(mockNamespace, []byte("account:"+phone)),
		Phone:             phone,
		TelegramUsername:  "new_mock_operator",
		TelegramFirstName: "Mock",
		TelegramLastName:  "Verified",
	}
	command.store.accounts = append(command.store.accounts, account)
	return account, nil
}
...
func (command StartQRCommand) Execute(ctx context.Context) (application.QRChallenge, error) {
	if failure := validateContext(ctx); failure != nil {
	...
	sequence := command.store.nextIDLocked("qr")
	challenge := application.QRChallenge{
		RequestID: uuid.NewSHA1(mockNamespace, []byte(sequence)),
		URL:       mockQRURL(sequence),
		ExpiresAt: time.Now().Add(qrChallengeLifetime),
	}
	command.store.qr = &qrChallengeState{challenge: challenge}
	return challenge, nil
}
...
func (command RefreshQRCommand) Execute(
	...
	if command.store.qr == nil || command.store.qr.challenge.RequestID != requestID {
		return application.QRChallenge{}, fmt.Errorf("%w: QR request %s", ErrChallengeNotFound, requestID)
	}
	if !time.Now().Before(command.store.qr.challenge.ExpiresAt) {
		command.store.qr = nil
		return application.QRChallenge{}, fmt.Errorf("%w: QR request %s", ErrChallengeExpired, requestID)
	}

	sequence := command.store.nextIDLocked("qr-refresh")
	refreshed := application.QRChallenge{
		RequestID: requestID,
		URL:       mockQRURL(sequence),
		ExpiresAt: time.Now().Add(qrChallengeLifetime),
	}
	command.store.qr.challenge = refreshed
...
func (request StatusRequest) Execute(ctx context.Context) (application.Status, error) {
	if failure := validateContext(ctx); failure != nil {
	...
	status := application.Status{
		Accounts: append([]application.Account(nil), request.store.accounts...),
	}
	if request.store.phone != nil {
		challenge := request.store.phone.challenge
		status.PhoneChallenge = &challenge
	}
	if request.store.qr != nil {
		challenge := request.store.qr.challenge
		status.QRChallenge = &challenge
	}
	return status, nil
}

func (store *Store) nextIDLocked(kind string) string {
	store.sequence++
	return fmt.Sprintf("%s:%d", kind, store.sequence)
}

func (store *Store) expireChallengesLocked(now time.Time) {
	if store.phone != nil && !now.Before(store.phone.challenge.ExpiresAt) {
		store.phone = nil
	}
	if store.qr != nil && !now.Before(store.qr.challenge.ExpiresAt) {
		store.qr = nil
	}
}

func mockQRURL(sequence string) string {
	return "tg://login?token=mock-" + strings.ReplaceAll(sequence, ":", "-")
}
